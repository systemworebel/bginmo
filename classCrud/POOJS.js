...
traerDatos.prototype.datosTraidos=function(){
  ...
  conexion1.open('GET','POO.php?solicitar=1&modificar=0&anadir=0&eliminar=0&archivotxt='+this.identificador+'.txt', true);
  conexion1.send(null);
...
inicializando.prototype.inicializado=function(){
	...
	conexion1=crearXMLHttpRequest();
  	...
  	conexion1.open('GET','POO.php?solicitar=0&modificar=0&anadir=0&eliminar=1&archivotxt='+identificador+'.txt', true);
  	conexion1.send(null);
	}
	...
	function verificarTextarea4(){
		...
		if(valor!=0){
			conexion1=crearXMLHttpRequest();
  			conexion1.onreadystatechange = procesarEventos;
  			conexion1.open('GET','POO.php?modificar='+div10.value+'&anadir=0&solicitar=0$eliminar=0&archivotxt='+identificador+'.txt', true);
  			conexion1.send(null);
		}else{
			alert('Debe ingresar algun dato');
			return 0;
